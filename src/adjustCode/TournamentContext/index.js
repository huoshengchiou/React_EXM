...
export const TournamentContainer = (props) => {
  ...
  useEffect(() => {
    ...
    if (
      fetchOnce.current &&
      AboutNextStatus.status === "registration_closed" &&
      AboutNextStatus.seconds_to_status === 0
    ) {
    ...
    if (FchBkT8tInfo?.t8t_staff) {
      ...
      if (t8t_staff.length !== 0) {
        let staffArr = t8t_staff.find(
          (item) => item.member_id === getMemberId()
        );
        if (staffArr) {
          isThisGameJudge.current = staffArr.role === "judge" ? true : false;
        }
      }
    }
    //-------^---Tournament qualification of user----^-----

    // setDaysBFStart(leftDays.current)
    //game status照跑//但timer關掉
  ...
  useEffect(() => {
    ...
    switch (GameStatus) {
      case "registration_not_open":
        ...
        setDefaultBannerMsg(
          intl.formatMessage({
            id: "Single-Tournament-Banner-Status_Registration will open soon",
          })
        );
        // setBtnClose(true);
        break;
      case "registration_opened":
        ...
        setDefaultBannerMsg(
          intl.formatMessage({
            id: "Single-Tournament-Banner-Status_Almost Full !! Hurry Up!",
          })
        );
        break;
      case "registration_closed":
        setRenderGameStatus(
          intl.formatMessage({
            id: "Single-Tournament-Banner-Status_Registration Closed",
          })
        );
        //不論註冊身分有無皆會顯示
        // setDefaultBannerMsg(`Begining in ${leftDays.current} days`)
        setDefaultBannerMsg(
          <FormattedMessage
            id={"Single-Tournament-Banner-Status_Beginning in {day} days"}
            values={{ day: leftDays.current }}
          />
        );
        break;
      case "ongoing":
        setRenderGameStatus(
          intl.formatMessage({
            id: "Single-Tournament-Banner-Status_Event Ongoing",
          })
        );
        // '%Connect Wording%' 072420
        setDefaultBannerMsg("");
        break;
      case "completed":
        setRenderGameStatus(
          intl.formatMessage({
            id: "Single-Tournament-Banner-Status_Completed",
          })
        );
        // '%Connect Wording%' 072420
        setDefaultBannerMsg("");
  ...
  useEffect(() => {
    ...
    if (VisitorStatus && !isIndividual) {
      setIsApplier(getMemberId() === VisitorStatus.t8tTeamApplier.member_id);
    }
    // 只處理registration_opened 期間狀態改變
    // if (GameStatus === 'registration_opened') {

    if (AboutNextStatus.status === "registration_opened") {
      if (isIndividual) {
        const finalStr = VisitorStatus.is_register_finish
          ? `${intl.formatMessage({
              id: "Single-Tournament-Banner-Status_[btn]Joined",
            })}▾`
          : intl.formatMessage({
              id: "Single-Tournament-Banner-Status_[btn]Go Registration",
            });
    ...
    if (AboutNextStatus.status === "registration_closed") {
      console.log("參賽身分找skin");
      //如果game本身不需要進行checkin
      //TODO 0716_add
      if (!gameNeedsCK.current) {
        setRenderRegStatus(
          intl.formatMessage({
            id: "Single-Tournament-Banner-Status_[btn]See Bracket",
          })
        );
      ...
      setRenderRegStatus(
        intl.formatMessage({
          id: "Single-Tournament-Banner-Status_[btn]Check-in",
        })
      );
      setUserBtnSkin("WaitForCk");
      // }
      // setBtnClose(true)
      // console.log('進入VisitorStatus skin判別')
    }
  }, [VisitorStatus]);
  ...
  const CallSecondTimer = (delay, callback) => {
    //  default block
    if (
      CheckInTime.current === 0 ||
      CheckETime.current === 0 ||
      fetchUserRegStatus.current
    )
      return;
    //run once close
  ...
  const ManagerTimer = () => {
    ...
    console.log(
      "狀態判斷",
      fetchUserRegStatus.current !== null && gameNeedsCK.current
    );
    if (fetchUserRegStatus.current !== null && gameNeedsCK.current) {
      // 當還有CK前有剩餘時間
      CheckInLeftTime.current
        ? CallSecondTimer(CheckInLeftTime.current)
        : CallSecondTimer(CheckInTime.current);
